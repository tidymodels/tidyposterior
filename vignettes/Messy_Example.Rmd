...
```{r rmse-results, eval = eval_chunks, svg = TRUE}
library(tidyposterior)
data("noisy_example")
...
ggplot(stacked_rmse, aes(col = Model, x = RMSE)) + 
...
```{r linear-linear, eval = eval_chunks, svg = TRUE}
...
```{r log-linear, eval = eval_chunks, svg = TRUE}
...
```{r gamma, eval = eval_chunks, svg = TRUE}
...
```{r gamma-contrast, eval = eval_chunks, svg = TRUE}
