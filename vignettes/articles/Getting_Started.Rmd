...
The previous spline recipe is altered so that the degrees of freedom parameter doesn't have an actual value. Instead, using a value of `tune()` will mark this parameter for optimization. There are a few different approaches for tuning this parameter; we'll use simple grid search. 
...
There is a `perf_mod()` method for this type of object. The computations are conducted in the same manner but, in this instance, three sub-models are compared. 
...
#| fig-alt: "Faceted line chart. Difference along the x-axis, posterior probability along the y-axis. Each density corresponds to the difference between preprocessor1 and the other 2 preprocessors.
