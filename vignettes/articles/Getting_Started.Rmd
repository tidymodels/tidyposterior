...
The main options in our analysis are passed through to the `brms` function `brm()`. These include: 
...
* `prior`: The main argument in this analysis for specifying the prior distribution of the parameters. 
...
The `summary()` function for this type of object shows the output from `brms::brm()`. It's long, so we show some of the initial output: 
...
The default family for `brm()` is Gaussian.  Given the characteristics of this metric, that assumption might seem problematic. 
...
Finally, a different distribution can be assumed using the family argument to `brm()`. Since our metrics are numeric, there are not many families to choose from. 
